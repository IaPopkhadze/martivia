        ...
        {questionAnswer.map((element, index) => (
          <div className="each_question_container-content" key={index} onClick={() => toggleAnswer(index)}>
            <div className="question">
              ...
              {selectedQuestionIndex === index ? <IoIosArrowUp className="arrow" /> : <IoIosArrowDown className="arrow" />}
            </div>
            <div className="answer" style={{ maxHeight: calculateAnswerHeight(index), overflow: "hidden", transition: "max-height 0.3s ease" }} ref={(ref) => (answerRefs.current[index] = ref)}>
              <div className="inside" style={{padding:'1rem'}}>{element.answer}</div>
            </div>
          </div>
        ))}
      </div>
